...
namespace OopLabs.Linq
{
    public partial class Form1 : Form
    {
        ...
        {
           ArtistsListBox.DataSource = Album.GetAlbums().Select(album => album.Artist).OrderBy(Artists => Artists).Distinct().ToList();
        }
        private void ArtistsListBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            AlbumsListBox.DataSource = Album.GetAlbums().Where(album => (album.Artist == ArtistsListBox.SelectedItem.ToString())).OrderByDescending(album => album.Date).ToList();
